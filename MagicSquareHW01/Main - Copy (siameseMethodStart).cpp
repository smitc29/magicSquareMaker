...
void main()
{
	...
	else
	{
		...
		if (value % 2 == 1) // Odd square logic
		{
			...
			board[row][col] = 1; // First node must be 1
			cout << "\n " << 1 << " has been placed...";

			for (int i = 1; i < count; i++)  // Begin placing items in nodes
			{
				...
				if ((row) >= value) // If node value is too low
					row = 0; // ...set it up one value

				if ((row) < 0) // If node value is too high
					row = value - 1; // ...set to lowest value in current grid

				if ((col) >= value) // If node value is too far right
					col = 0; // ...sets to farthest left nodes
